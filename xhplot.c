...
waitseconds(void)
{
	...
	scanf("%d",&seconds);
	//sleep(seconds);
	sprintf(cmd,"/bin/sleep %d",seconds);
	system (cmd);
}

//void pointer(double xfactor, double yfactor,
pointer(double xfactor, double yfactor,
...
point(double xfactor, double yfactor, 
			int sv[], double sc[], double *x, double *y)
//point(xfactor,yfactor,sv,sc,x,y)
//	int sv[];
//	double xfactor,yfactor,sc[];
...
cylpoint(double xfactor, double yfactor, 
				int sv[], double sc[], double *x, double *y)
//cylpoint(xfactor,yfactor,sv,sc,x,y)
//	int sv[];
//	double xfactor,yfactor,sc[];
...
readbitmap(unsigned int *width, unsigned int *height, Pixmap *bitmap, int *xhot, int *yhot)
//unsigned int *width,*height;
//Pixmap *bitmap;
//int *xhot,*yhot;
{
...
arccoordinate(int ix, int iy, double xfactor, double yfactor,
			double *x, double *y, int sv[], double sc[])
//arccoordinate(ix,iy,xfactor,yfactor,x,y,sv,sc)
//int ix,iy,sv[];
//double xfactor,yfactor,*x,*y,sc[];
...
arccylcoordinate(int ix, int iy, double xfactor, double yfactor,
			double *x, double *y, int sv[], double sc[])
//arccylcoordinate(ix,iy,xfactor,yfactor,x,y,sv,sc)
//	int ix,iy,sv[];
//	double xfactor,yfactor,*x,*y,sc[];
...
line(int ix0,int iy0,int ix,int iy)
...
setarcstruct(unsigned int *width, unsigned int *height,
				int *angle1, int *angle2)
//setarcstruct(width,height,angle1,angle2)
//unsigned int *width,*height;
//int *angle1,*angle2;
{
#ifdef VERBOSE
...
arc(int ix, int iy, unsigned int width,
			unsigned int height, int angle1,int angle2)
//arc(int ix,int iy,unsigned int width,unsigned int height,int angle1,int angle2)
...
data(int sv[], double sc[], double *xfactor,
...
axes_data(double la[],int lav[], int fi[], int *kse,
...
#endif
		if(scanf("%d %d",&fi[0],&fi[1])!= 2) getchar();
		//scanf("%d %d",&fi[0],&fi[1]) 
		//!= 2) getchar();
#ifdef VERBOSE
		printf("AXES(1) or GRID(2) [%d]: ",*kse);
#endif
		if(scanf("%d",kse)!= 1) getchar();
		//scanf("%d",kse) 
		//!= 1) getchar();
		setfont();
#ifdef VERBOSE
...
axes(int sv[], int lav[], int fi[], int *kse,
...
cylaxes(int sv[], int lav[], int fi[], int *kse,
...
//char s[];
{
	char *strcpy(),*fcvt();
	char str[CHARLEN];
	int i = 0,j = 0,dec,ndec,sign,ndig;
	if (fabs(val)*pow(10.0,(double)ndigit) < .9) {
		s[0] = '0'; s[1] = '\0';
		return;
	}
        strcpy(str,fcvt(val,ndigit,&dec,&sign));
        if (sign == 1) s[i++] = '-';
	if (dec <= 0) s[i++] = '0';
        if (dec < 0) {
                s[i++] = '.';
...
moji(int ise, int jfx, int lbl, int lt, double t, int flag_ur)
...
set_arrow(int *ver, int *head, int *inv)
...
arrow(int ix, int iy, int ver, int head, int inv)
...
draw_arrow(int i, int s, int ix, int iy)
...
drawstrings(int x, int y, char string[])
...
set_misc(int sv[], double sc[], double *xfactor,
...
struct2data(struct xddp_dat xddp, 
